...
main_agent.py (Orchestrator)
  ...
  ├── [2] src/agent_2_editor.py      → AI-enhanced edit (editor/ai_editor.py) with voiceover + subtitles + red hook circle on a 9:16 branding frame
...
1. **Video Analysis** — PySceneDetect scene detection + faster-whisper transcription. If the video is longer than 59s, the LLM selects the most engaging crop window.
...
7. **Audio Mix** — TTS voiceover plays over the video, with the original audio swapped back in for 5 seconds in the middle.
